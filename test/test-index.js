...
// An example tree representing an interactive version of The Matrix.
let tree = new Tree({
    tree: {
        root: new TreeNode(
            function(input) {
...
// An example tree representing an interactive version of The Stranger.
let asyncTree = new Tree({
    tree: {
        root: new TreeNode(
            function (input) {
                console.log("Take the red pill or the blue pill?");
...
describe('Tree Test', function () {
    ...
    it('Check to see if tree fallbacks work', function () {
        let treeWalk = tree.start(["red", "nevermind"]);
        console.log(treeWalk);
        assert.equal(treeWalk.output, "There was a problem.", 'If an error is thrown inside the tree, call the fallback');
    });
...
describe('Async Tree Test', function () {
    it('Check to see if the tree iterates correctly (option 1)', function () {
        assert.equal(tree.start(["red"]).output, "You see the truth.", 'If you input "red" and leave the Matrix, you should see the truth');
    });
    it('Check to see if the tree iterates correctly (option 2)', function () {
